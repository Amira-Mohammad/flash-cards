...
const StartQuiz = ({ route, navigation }) => {
    ...
    const correctAnswer = availableCard.questions[status === "next" ? next : previous].answer;
    const wrongAnswer = availableCard.questions[status === "next" ? next : previous].wrongAnswer;
    const data = [
        {
            label: correctAnswer
        },
        ...
        <View style={styles.container}>
            {availableCard.questions.length > 0 ?
                ...
                <View>
                    ...
                    <Text>{availableCard.questions[status === "next" ? next : previous].answer}</Text>
                    <Text>{availableCard.questions[status === "next" ? next : previous].wrongAnswer}</Text>
                    ...
                    <View>
                        <TouchableOpacity style={styles.showAnswer}
                            onPress={() => {
                                setShow(!show)
                            }}>
                    ...
                    <TouchableOpacity style={styles.startOver}
                        onPress={() => {

                            console.log("start the quiz over")
                        }}
                    ><Text>start the quiz over</Text></TouchableOpacity>
                </View>
                : <View>fffffffff</View>}
        </View>
    );
